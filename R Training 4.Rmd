...
snack_data %>%
  group_by(Q2_GENDER) %>%
  summarise_at(vars(num_range("PRELUN_", 1:3)), ~sum(.)/length(.)) # can also use funs() form, depending on dplyr version

snack_data %>%
  group_by(Q2_GENDER) %>%
  summarise_at(vars(num_range("POSLUN_", 1:3)), ~sum(.)/length(.))

snack_data %>%
  group_by(Q2_GENDER) %>%
  summarise_at(vars(num_range("LATAFT_", 1:3)), ~sum(.)/length(.))
```
